...
if (is_dir(PRODUCTION_BK_PATH)) {
    ...
    if (is_dir(PRODUCTION_BK_PATH)) {
        exit('ERROR - Ancienne version non supprimé');
    }
}
...
if (!rename(PRODUCTION_PATH, PRODUCTION_BK_PATH)) {
    exit('ERROR - Ancienne version non déplacé');
}
...
if (!rename(PRODUCING_PATH, PRODUCTION_PATH)) {
    exit('ERROR - Nouvelle version non déplacé');
}
